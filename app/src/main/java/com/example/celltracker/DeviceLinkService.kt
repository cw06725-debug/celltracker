...
class DeviceLinkService : Service() {
    ...
    private fun startControllerTest(config: CallSetupConfig) {
        ...
        if(local.phoneForSlot(config.aCallSimSlot).isBlank() || peer?.phoneForSlot(config.bCallSimSlot).isNullOrBlank()){ setLinkError("Configure the selected Call SIM phone numbers on both DUTs first"); return }
    ...
    private suspend fun runAttempt(number:Int,total:Int,direction:String,a:DeviceProfile,b:DeviceProfile,c:CallSetupConfig,session:String):CallAttemptResult {
        ...
        if(failure.isBlank()) {
            ...
            if(direction=="A_TO_B") {
                ...
                if(!placeCall(b.phoneForSlot(c.bCallSimSlot),c.aCallSimSlot)) failure="Unable to place MO call"
            } else send("DIAL",session,attemptId,mapOf("number" to a.phoneForSlot(c.aCallSimSlot)))
        }
