        ...
        val samples = lines.drop(1).mapNotNull { line ->
            runCatching {
                ...
                TrackSample(
                    ...
                    cqi = field(f, "cqi").ifBlank { "--" },
                    level = field(f, "signal_level").ifBlank { "--" },
                    asuLevel = field(f, "asu_level").ifBlank { "--" },
                    carrierAggregation = field(f, "carrier_aggregation").ifBlank { "--" },
                    dataRat = field(f, "data_rat").ifBlank { "--" },
                    voiceRat = field(f, "voice_rat").ifBlank { "--" },
                    roaming = field(f, "roaming").ifBlank { "--" },
                    latitude = lat,
                    longitude = lon,
